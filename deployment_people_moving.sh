...
docker run --cpus="8" --ulimit nproc=8 --memory="2000m" --network="host" --log-opt max-size=10m --log-opt max-file=3 -e CCTV_AREA="stb" --name people_moving_stb --gpus all -d --restart unless-stopped people_moving:$NV